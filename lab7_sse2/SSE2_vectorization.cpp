...
// FIXME: write a separate class for matrix.
float* MatrixMultiply(float* first, const float* second, int matrixSize)
{
    auto result = (float*) _mm_malloc(matrixSize * matrixSize * sizeof(float), 16);
    for (int i = 0; i < matrixSize; ++i)
    {
        float* resultMatrix = first + i * matrixSize;
        for (int k = 0; k < matrixSize; ++k)
        {
            const float* secondMatrix = second + k * matrixSize;
            float firstMatrix = first[i*matrixSize + k];
            for (int j = 0; j < matrixSize; ++j)
            {
