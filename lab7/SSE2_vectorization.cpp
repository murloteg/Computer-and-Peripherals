...
void SetMatrix(float* matrix, int matrixSize)
{
	...
	for (int i = 0; i < matrixSize; ++i)
	{
		...
		{
			matrix[i * matrixSize + j] = static_cast<float> (number % 5);
			++number;
		}
	}
}
...
float* TransposeMatrix(const float* matrix, int matrixSize)
{
	...
	for (int i = 0; i < matrixSize; ++i)
	{
		for (int j = 0; j < matrixSize; ++j)
		{
			transposedMatrix[i * matrixSize + j] = matrix[i * matrixSize + j];
		}
	}
...
float* MatrixMultiply(const float* first, const float* second, int matrixSize)
{
	...
	for (int k = 0; k < matrixSize; ++k)
	{
		for (int i = 0; i < matrixSize; ++i)
		{
			__m128 currentSum = _mm_setzero_ps();
			for (int j = 0; j < matrixSize / 4; ++j)
			{
				__m128 firstValue = _mm_load_ps(first + i * matrixSize / 4 + k);
				__m128 secondValue = _mm_load_ps(second + k * matrixSize / 4 + j);
				__m128 multResult = _mm_mul_ps(firstValue, secondValue);
				currentSum = _mm_add_ps(currentSum, multResult);
			}
			_mm_store_ps(result + i * matrixSize / 4 + k, currentSum);
		}
	}
